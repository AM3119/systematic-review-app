...
export default function ReviewLayout() {
  ...
  const { data: progress } = useQuery(['progress', reviewId], () => reviewsApi.stats(reviewId!).then(r => r.data), {
    enabled: !!reviewId, refetchInterval: 30000
  });
  ...
  const navItems = [
    ...
    { to: `/reviews/${reviewId}/conflicts`, icon: ExclamationTriangleIcon, label: 'Conflicts', badge: progress?.conflicts },
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
        ...
        <div className="p-4 border-b border-gray-100">
          <button onClick={() => navigate('/')} className="flex items-center gap-2 text-gray-500 hover:text-gray-900 text-sm mb-3 transition-colors">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-semibold text-gray-900 truncate leading-tight mt-0.5">{review?.title || '...'}</p>
            </div>
          </div>
          {review?.blinding_enabled ? (
            <div className="mt-2 flex items-center gap-1.5 text-xs text-amber-700 bg-amber-50 px-2 py-1 rounded-md">
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {navItems.map(({ to, icon: Icon, label, end, badge }) => (
            <NavLink key={to} to={to} end={end}
              ...
              {badge ? <span className="bg-red-100 text-red-700 text-xs font-bold px-1.5 py-0.5 rounded-full">{badge}</span> : null}
            </NavLink>
          ))}
        </nav>
        ...
        <div className="p-3 border-t border-gray-100">
          <div className="flex items-center gap-3 px-2 py-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-xs text-brand-600 font-medium">{user?.points || 0} pts</span>
                {(user?.streak || 0) > 0 && (
            ...
            <div className="relative">
              <button onClick={() => setNotifOpen(o => !o)} className="relative p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
              ...
              {notifOpen && (
                <div className="absolute bottom-full right-0 mb-2 w-72 bg-white rounded-xl shadow-xl border border-gray-200 z-50 max-h-80 overflow-y-auto">
                  ...
                  {!notifications?.length ? (
                    ...
                    <div key={n.id} className={`p-3 border-b border-gray-50 text-sm ${!n.read ? 'bg-brand-50' : ''}`}>
          ...
          <button onClick={() => { logout(); navigate('/login'); }}
            className="mt-1 w-full text-left px-3 py-1.5 text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg transition-colors">
            Sign out
          </button>
        </div>
      </aside>
