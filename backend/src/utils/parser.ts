...
export function parseRIS(content: string): ParsedArticle[] {
  ...
  for (const record of records) {
    ...
    for (const line of lines) {
      ...
      switch (tag) {
        case 'TI': case 'T1': case 'CT': article.title = article.title || v; break;
        case 'AU': case 'A1': case 'A2': authorsList.push(v); break;
        case 'AB': case 'N2': article.abstract = article.abstract || v; break;
        case 'JO': case 'JF': case 'J1': case 'J2': case 'T2': article.journal = article.journal || v; break;
        case 'PY': case 'Y1': case 'DA': {
          const yr = parseInt(v.substring(0, 4));
          if (!isNaN(yr)) article.year = yr;
          break;
        }
        case 'VL': article.volume = v; break;
        case 'IS': article.issue = v; break;
        case 'SP': article.pages = v + (article.pages ? `-${article.pages}` : ''); break;
        case 'EP': article.pages = article.pages ? `${article.pages}-${v}` : v; break;
        case 'DO': case 'DI': article.doi = v; break;
        case 'AN': if (v.startsWith('PMID:')) article.pmid = v.replace('PMID:', '').trim(); break;
        case 'UR': case 'L1': article.url = article.url || v; break;
        case 'DB': article.source_db = v; break;
        case 'KW': case 'DE': keywordsList.push(v); break;
      }
    }

    if (authorsList.length) article.authors = authorsList.join('; ');
    if (keywordsList.length) article.keywords = keywordsList.join('; ');
...
export function parseBibTeX(content: string): ParsedArticle[] {
  const articles: ParsedArticle[] = [];
...
export function parseCSV(content: string): ParsedArticle[] {
